...
```
[1] compile+import (runtime absent): 107 compiled, 107 imported
...
| Exercise | Result |
|---|---|
| Every CLI entry point `--help`, **without** the runtime | all entry points resolve |
...
| **Evaluation end to end** against a local endpoint | tasks selected, executed and scored; strict scores and per-domain breakdown written to JSON |
...
| Runtime absent | modules that need it raise one explanatory error rather than an import error from deep inside a library |
...
`ASSETFORGE_RUNTIME_PIN`. Model credentials come from the environment. Steps that need the
runtime — compiling a task, executing a review — say so plainly when it is absent; everything
else runs.
